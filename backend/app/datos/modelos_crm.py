...
class CrmPipeline(TenantAuditMixin, Base):
    ...
    # Relaciones
    etapas = relationship(
        "CrmStage", back_populates="pipeline", order_by="CrmStage.orden", cascade="all, delete-orphan"
    )
    deals = relationship("CrmDeal", back_populates="pipeline")

    __table_args__ = (Index("idx_crm_pipeline_tenant_default", "tenant_id", "es_default"),)
...
class CrmActivity(TenantAuditMixin, Base):
    ...
    __table_args__ = (Index("idx_crm_activities_tenant_deal_fecha", "tenant_id", "deal_id", "fecha_actividad"),)
