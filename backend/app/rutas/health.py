...
async def readiness_check(db: Session = Depends(get_db)):
    ...
    try:
        ...
        if db_latency > 1.0:
            return Response(
                content='{"status":"degraded","database":"slow","db_latency_ms":'
                + str(round(db_latency * 1000, 2))
                + "}",
                status_code=503,
                media_type="application/json",
            )

        return {"status": "ready", "database": "healthy", "db_latency_ms": round(db_latency * 1000, 2)}

    except Exception as e:
        return Response(
            content=f'{{"status":"not_ready","error":"{str(e)}"}}', status_code=503, media_type="application/json"
        )
...
async def startup_check(db: Session = Depends(get_db)):
    ...
    try:
        ...
        return {"status": "started", "database": "connected", "migrations": "completed"}

    except Exception as e:
        return Response(
            content=f'{{"status":"not_started","error":"{str(e)}"}}', status_code=503, media_type="application/json"
        )
