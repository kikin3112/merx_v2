...
def generar_numero_secuencia(db: Session, nombre_secuencia: str) -> str:
    """
    Genera el siguiente número de una secuencia.
    ...
    secuencia = (
        db.query(Secuencias)
        .filter(Secuencias.nombre == nombre_secuencia)
        .with_for_update()
        .first()
    )

    if not secuencia:
        raise ValueError(f"Secuencia '{nombre_secuencia}' no encontrada en la base de datos")
...
def crear_secuencia_si_no_existe(
    ...
    """
    existe = db.query(Secuencias).filter(Secuencias.nombre == nombre).first()
...
def obtener_siguiente_numero_preview(db: Session, nombre_secuencia: str) -> str:
    """
    Obtiene una vista previa del siguiente número SIN incrementar la secuencia.
    Útil para mostrar al usuario qué número se asignará.
    ...
    if not secuencia:
        raise ValueError(f"Secuencia '{nombre_secuencia}' no encontrada")
