...
from ..datos.esquemas import UsuarioCreate, UsuarioUpdate, UsuarioResponse
    ...
    try:
        ...
        if 'email' in update_data:
            ...
            if existing:
                raise HTTPException(
                    status_code=status.HTTP_400_BAD_REQUEST,
                    detail=f"Email '{update_data['email']}' ya está en uso"
                )
...
async def reactivar_usuario(
    ...
    **Permisos:** Solo admin
    ...
    if current_user.rol != 'admin':
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Solo administradores pueden reactivar usuarios"
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail="Error interno al reactivar el usuario"
        )