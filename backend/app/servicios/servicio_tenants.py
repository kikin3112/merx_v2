...
from ..datos.modelos import Usuarios, Productos, Ventas, Terceros
from ..datos.modelos_tenant import (
    Planes, Tenants, UsuariosTenants, Suscripciones, HistorialPagos
)
...
class ServicioTenants:
    ...
    def obtener_planes_activos(self) -> List[Planes]:
        ...
        return self.db.query(Planes).filter(
            Planes.esta_activo == True
        ).order_by(Planes.precio_mensual).all()
    ...
    def crear_tenant(
        ...
        admin_existente = self.db.query(Usuarios).filter(
            Usuarios.email == datos.admin_email
        ).first()
        if admin_existente:
    ...
    def obtener_tenants_usuario(self, usuario_id: UUID) -> List[UsuariosTenants]:
        ...
        return self.db.query(UsuariosTenants).filter(
    ...
    def agregar_usuario_a_tenant(
        ...
        if rol == 'admin':
            ...
            if admin_existente:
                raise ValueError(
                    "Este tenant ya tiene un usuario con rol admin. "
                    "Solo se permite un admin por tenant."
                )
        ...
        if es_default:
            self.db.query(UsuariosTenants).filter(
                UsuariosTenants.usuario_id == usuario_id,
                UsuariosTenants.es_default == True
    ...
    def validar_acceso_tenant(
        ...
        usuario_tenant = self.db.query(UsuariosTenants).filter(
            UsuariosTenants.usuario_id == usuario_id,
            UsuariosTenants.tenant_id == tenant_id,
            UsuariosTenants.esta_activo == True
        ).first()
    ...
    def listar_todos_planes(self) -> List[dict]:
        ...
        resultado = []
        for plan in planes:
            tenant_count = self.db.query(sqla_func.count(Tenants.id)).filter(
                Tenants.plan_id == plan.id,
    ...
    def actualizar_plan(self, plan_id: UUID, datos: PlanUpdate) -> Optional[Planes]:
        ...
        if "nombre" in update_data and update_data["nombre"] != plan.nombre:
            existente = self.db.query(Planes).filter(
                Planes.nombre == update_data["nombre"],
                Planes.id != plan_id
            ).first()
            if existente:
    ...
    def desactivar_plan(self, plan_id: UUID) -> Optional[Planes]:
        ...
        if tenant_count > 0:
            raise ValueError(
                f"No se puede desactivar el plan '{plan.nombre}': "
                f"tiene {tenant_count} tenant(s) activo(s)"
            )
    ...
    def obtener_metricas_tenant(self, tenant_id: UUID) -> dict:
        ...
        usuarios_count = self.db.query(sqla_func.count(UsuariosTenants.id)).filter(
            UsuariosTenants.tenant_id == tenant_id,
            UsuariosTenants.esta_activo == True
        ).scalar() or 0

        productos_count = self.db.query(sqla_func.count(Productos.id)).filter(
            Productos.tenant_id == tenant_id
        ).scalar() or 0

        terceros_count = self.db.query(sqla_func.count(Terceros.id)).filter(
        ...
        facturas_mes = self.db.query(sqla_func.count(Ventas.id)).filter(
        ...
        ventas_mes_rows = self.db.query(Ventas).filter(
            Ventas.tenant_id == tenant_id,
            Ventas.created_at >= inicio_mes,
            Ventas.estado.in_(["CONFIRMADA", "FACTURADA"])
        ).all()
    ...
    def obtener_saas_kpis(self) -> dict:
        ...
        activos = self.db.query(sqla_func.count(Tenants.id)).filter(
            Tenants.estado == "activo"
        ).scalar() or 0
        trial = self.db.query(sqla_func.count(Tenants.id)).filter(
            Tenants.estado == "trial"
        ).scalar() or 0
        suspendidos = self.db.query(sqla_func.count(Tenants.id)).filter(
            Tenants.estado == "suspendido"
        ).scalar() or 0
        cancelados = self.db.query(sqla_func.count(Tenants.id)).filter(
            Tenants.estado == "cancelado"
        ).scalar() or 0

        # Nuevos últimos 30 días
        hace_30_dias = datetime.now(timezone.utc) - timedelta(days=30)
        nuevos_30d = self.db.query(sqla_func.count(Tenants.id)).filter(
            Tenants.created_at >= hace_30_dias
        ).scalar() or 0

        # MRR: sum precio_mensual de planes con tenants activos
        mrr_result = self.db.query(
            sqla_func.sum(Planes.precio_mensual)
        ).join(
            Tenants, Tenants.plan_id == Planes.id
        ).filter(
            Tenants.estado.in_(["activo", "trial"])
        ).scalar()
        ...
        cancelados_30d = self.db.query(sqla_func.count(Tenants.id)).filter(
            Tenants.estado == "cancelado",
            Tenants.updated_at >= hace_30_dias
        ).scalar() or 0
        ...
        for plan in planes:
            count = self.db.query(sqla_func.count(Tenants.id)).filter(
                Tenants.plan_id == plan.id,
                Tenants.estado.in_(["activo", "trial"])
            ).scalar() or 0
    ...
    def obtener_usuarios_tenant_con_detalle(self, tenant_id: UUID) -> List[dict]:
        ...
        resultados = self.db.query(
            UsuariosTenants, Usuarios
        ).join(
            Usuarios, Usuarios.id == UsuariosTenants.usuario_id
        ).filter(
            UsuariosTenants.tenant_id == tenant_id,
            UsuariosTenants.esta_activo == True
        ).all()
    ...
    def obtener_suscripciones_tenant(self, tenant_id: UUID) -> List[Suscripciones]:
        ...
        return self.db.query(Suscripciones).filter(
            Suscripciones.tenant_id == tenant_id
        ).order_by(Suscripciones.created_at.desc()).all()
    ...
    def obtener_pagos_tenant(self, tenant_id: UUID) -> List[HistorialPagos]:
        ...
        suscripcion_ids = self.db.query(Suscripciones.id).filter(
            Suscripciones.tenant_id == tenant_id
        ).subquery()
