...
async def crear_tercero(tercero_data: TerceroCreate, db: Session = Depends(get_db),
                        current_user: Usuarios = Depends(get_current_user)):
    if current_user.rol not in ['admin', 'operador']:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Sin permisos")

    existing = db.query(Terceros).filter(Terceros.numero_documento == tercero_data.numero_documento).first()
    if existing:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail=f"Documento {tercero_data.numero_documento} ya existe")

    try:
        nuevo_tercero = Terceros(**tercero_data.model_dump())
...
async def listar_terceros(skip: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=1000),
                          tipo_tercero: Optional[str] = Query(None), estado: Optional[bool] = Query(None),
                          busqueda: Optional[str] = Query(None), db: Session = Depends(get_db),
                          current_user: Usuarios = Depends(get_current_user)):
    query = db.query(Terceros)
    if tipo_tercero:
...
async def obtener_tercero(tercero_id: UUID, db: Session = Depends(get_db),
                          current_user: Usuarios = Depends(get_current_user)):
...
async def actualizar_tercero(tercero_id: UUID, tercero_data: TerceroUpdate,
                             db: Session = Depends(get_db), current_user: Usuarios = Depends(get_current_user)):
    if current_user.rol not in ['admin', 'operador']:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Sin permisos")
...
async def eliminar_tercero(tercero_id: UUID, db: Session = Depends(get_db),
                           current_user: Usuarios = Depends(get_current_user)):
